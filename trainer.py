...
class Trainer:
    def __init__(self, model, tmodel, args, supervised_loader, unsupervised_loader, val_loader, iter_per_epoch, writer, dataset_len, undataset_len, tmp_path):
        ...
        self.iqa_psnr =  pyiqa.create_metric('psnr',  as_loss=False)
        self.iqa_ssim =  pyiqa.create_metric('ssim',  as_loss=False)
        self.iqa_niqe = pyiqa.create_metric('musiq', as_loss=False)


        self.score_predict_ul = [0 for _ in range(self.undataset_len)]
        ...
        self.Fz_unlbl = torch.zeros((self.undataset_len, self.z_numchnls, self.z_height, self.z_width),
        ...
        self.initial_un_featmaps(self.unsupervised_loader, self.model, self.tmodel)

    def initial_un_featmaps(self, un_dataloader, s_net, t_net):
        print("initializing unlabeled latent vectors........")
        u_count = 0
        for batch_id, train_data in enumerate(un_dataloader):
            u_input_im, imgid = train_data
            u_input_im = u_input_im.to(self.device)


            s_pred_image, s_zy_in = s_net(u_input_im)
            t_pred_image, t_zy_in = t_net(u_input_im)

            score_s = -self.iqa_niqe(s_pred_image[0])
            score_t = -self.iqa_niqe(t_pred_image[0])

            shape = t_zy_in.data.shape[0]  # batch
            for i in range(shape):
                new_imgid = os.path.join(self.tmp_path, imgid[i].split('/')[-1])
                if new_imgid not in self.dict_unlbl.keys():
                    self.dict_unlbl[new_imgid] = u_count
                    u_count += 1
                tmp_i = self.dict_unlbl[new_imgid]

                if score_s[i] < score_t[i]:
                    self.Fz_unlbl[tmp_i, :, :, :] = s_zy_in.data[i, :, :, :].data
                    self.score_predict_ul[tmp_i] = score_s[i]

                    temp_un = torchvision.transforms.ToPILImage()(s_pred_image[0][i, :].cpu())
                    temp_un.save(new_imgid)
                else:
                    self.Fz_unlbl[tmp_i, :, :, :] = t_zy_in.data[i, :, :, :].data
                    self.score_predict_ul[tmp_i] = score_t[i]
                    temp_un = torchvision.transforms.ToPILImage()(t_pred_image[0][i, :].cpu())
                    temp_un.save(new_imgid)
    ...
    def train(self):
        ...
        gp_struct.initial_featmaps(self.supervised_loader, self.model, self.tmodel)
        best_val_psnr = 0
    ...
    def _train_epoch(self, epoch, gp_struct):
        ...
        for i in tbar:
            ...
            loss_sup = structure_loss + 0.4 * perpetual_loss + 0.6*ssim_loss
            sup_loss.update(loss_sup.mean().item())

            pseudo_latent_u, pseudo_predict_u = self.get_reliable(predict_target_u, predict_latent_u, outputs_ul, latent_ul, unimg_id)
            pseudo_latent_u, pseudo_predict_u = pseudo_latent_u.detach().clone(), pseudo_predict_u.detach().clone()
            unsu_gp_loss = gp_struct.compute_gploss(latent_ul, unimg_id, pseudo_latent_u, label_flg=1)

            unsu_structure_loss = self.loss_single_recon(pseudo_predict_u, outputs_ul)
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for i, (val_data, val_label, hs, ws, image_name, label_path) in enumerate(tbar):
                ...
                temp_psnr = self.iqa_psnr(f'./logs/val_image/{image_name[0]}', label_path[0]).item()
                temp_ssim = self.iqa_ssim(f'./logs/val_image/{image_name[0]}', label_path[0]).item()
    ...
    def get_reliable(self, predict_target_u, predict_latent_u, outputs_ul, latent_ul, img_id):
        N = predict_latent_u.shape[0]
        score_t = -self.iqa_niqe(predict_target_u[0]).detach().cpu().numpy()
        score_s = -self.iqa_niqe(outputs_ul[0]).detach().cpu().numpy()
        pseudo_predict = torch.zeros_like(predict_target_u[0]).cuda()
        pseudo_latent = torch.zeros_like(predict_latent_u).cuda()

        for idx in range(0, N):
            if score_t[idx] > score_s[idx]:
                new_imgid = os.path.join(self.tmp_path, img_id[idx].split('/')[-1])
                tmp_i = self.dict_unlbl[new_imgid]
                if score_t[idx] > self.score_predict_ul[tmp_i]:
                    # update the pam_2
                    self.score_predict_ul[tmp_i] = score_t[idx]
                    self.Fz_unlbl[tmp_i] = predict_latent_u[0][idx]
                    tmp_predict = torch.clamp(predict_target_u[0][idx, :], 0, 1)
                    temp_un = torchvision.transforms.ToPILImage()(tmp_predict.cpu())
                    temp_un.save(new_imgid)
                    pseudo_predict[idx] = predict_target_u[0][idx]
                    pseudo_latent[idx] = predict_latent_u[idx]
                else:
                    pseudo_predict[idx] = TF.to_tensor(Image.open(new_imgid))
                    pseudo_latent[idx] = self.Fz_unlbl[tmp_i]
            else:
                new_imgid = os.path.join(self.tmp_path, img_id[idx].split('/')[-1])
                tmp_i = self.dict_unlbl[new_imgid]
                if score_s[idx] > self.score_predict_ul[tmp_i]:
                    # update the pam_2
                    self.score_predict_ul[tmp_i] = score_s[idx]
                    self.Fz_unlbl[tmp_i] = latent_ul[idx]
                    tmp_predict = torch.clamp(outputs_ul[0][idx, :], 0, 1)
                    temp_un = torchvision.transforms.ToPILImage()(tmp_predict.cpu())
                    temp_un.save(new_imgid)
                    pseudo_predict[idx] = outputs_ul[0][idx]
                    pseudo_latent[idx] = latent_ul[idx]
                else:
                    pseudo_predict[idx] = TF.to_tensor(
                        Image.open(new_imgid))
                    pseudo_latent[idx] = self.Fz_unlbl[tmp_i]
        del N, score_s, score_t, predict_target_u, predict_latent_u, outputs_ul, latent_ul
        return pseudo_latent, pseudo_predict