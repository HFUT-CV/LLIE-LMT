...
class GPStruct(object):
    def __init__(self, num_lbl, num_unlbl, train_batch_size, device):
        ...
        self.iqa_psnr = pyiqa.create_metric('psnr', device=self.device, as_loss=False)

        self.score_latent_l = [0 for _ in range(self.num_lbl)]
    def initial_featmaps(self, su_dataloader, s_net, t_net):
        print("initializing labeled latent vectors........")
        s_count = 0
        for batch_id, train_data in enumerate(su_dataloader):
            s_input_im, gt, imgid = train_data
            s_input_im = s_input_im.to(self.device)
            gt = gt.to(self.device)

            s_pred_image, s_zy_in = s_net(s_input_im)
            t_pred_image, t_zy_in = t_net(s_input_im)

            score_s = self.iqa_psnr(s_pred_image[0], gt)
            score_t = self.iqa_psnr(t_pred_image[0], gt)

            shape = t_zy_in.data.shape[0]  # batch
            for i in range(shape):
                if imgid[i] not in self.dict_lbl.keys():
                    self.dict_lbl[imgid[i]] = s_count
                    s_count += 1
                tmp_i = self.dict_lbl[imgid[i]]

                if score_s[i] < score_t[i]:
                    self.Fz_lbl[tmp_i, :, :, :] = t_zy_in.data[i, :, :, :].data
                    self.score_latent_l[tmp_i] = score_t[i]
                else:
                    self.Fz_lbl[tmp_i, :, :, :] = s_zy_in.data[i, :, :, :].data
                    self.score_latent_l[tmp_i] = score_s[i]
    def gen_featmaps(self, dataloader, s_net, t_net):
        ...
        for batch_id, train_data in enumerate(dataloader):
            ...
            t_score = self.iqa_psnr(t_pred_image[0], gt)
            s_score = self.iqa_psnr(s_pred_image[0], gt)
            ...
            for i in range(shape):
                ...
                if t_score[i] > s_score[i]:
                    if t_score[i] > self.score_latent_l[tmp_i]:
                        self.Fz_lbl[tmp_i, :, :, :] = t_zy_in.data[i, :, :, :].data
                else:
                    if s_score[i] > self.score_latent_l[tmp_i]:
                        self.Fz_lbl[tmp_i, :, :, :] = s_zy_in.data[i, :, :, :].data
